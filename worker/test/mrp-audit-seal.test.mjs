...
test('release persistence is fail-safe and seals before updating controlled state',async()=>{const source=await readFile(new URL('../src/mrp.js',import.meta.url),'utf8'),sealWrite=source.indexOf('INSERT OR IGNORE INTO mrp_audit_seals'),stateWrite=source.indexOf('UPDATE mrp_state');assert.ok(sealWrite>0);assert.ok(stateWrite>sealWrite);assert.match(source,/id=`MRPSEAL-\$\{signature\}`/);});

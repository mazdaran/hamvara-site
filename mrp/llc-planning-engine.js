...
function itemRecord(state,sku,itemByCode){return itemByCode?.get(sku)||(state.skus||[]).find(item=>item.code===sku)||{}}
function productFor(graph,sku){return graph.productByItem.get(sku)||null}
function leadDays(state,graph,sku,itemByCode){
  const product=productFor(graph,sku),item=itemRecord(state,sku,itemByCode);
  if(product){
...
export function llcMaterialPlanRows(state,orders,{asOf,cutoff}={}){
  const graphs=new Map(),balances=new Map(),supplyQueues=new Map(),pendingByLevel=new Map(),rows=[],itemByCode=new Map((state.skus||[]).map(item=>[item.code,item]));
  const enqueue=demand=>{const level=demand.llc||0;if(!pendingByLevel.has(level))pendingByLevel.set(level,[]);pendingByLevel.get(level).push(demand)};
  ...
  const enqueueChildren=(graph,parentItem,quantity,needDate,source)=>{
    ...
    for(const edge of edges)enqueue({...source,graph,sku:edge.childItem,warehouse:edge.warehouse,grossRequired:round(quantity*edge.quantity*(1+edge.wastePercent/100)),needDate,llc:graph.lowLevelCode.get(edge.childItem)||0,phantom:edge.phantom});
  };
  for(const order of orders){
    try{
      const graph=graphFor(order.due||asOf),product=graph.productByItem.get(order.productCode);
      if(!product)throw new Error(`Product not found for ${order.productCode}.`);
      const rootItem=product.outputSku||product.code,rootNeed=order.due||cutoff,componentNeed=addDays(rootNeed,-leadDays(state,graph,rootItem,itemByCode));
  ...
  while(pendingByLevel.size){
    const level=Math.min(...pendingByLevel.keys()),batch=pendingByLevel.get(level).sort((a,b)=>a.needDate.localeCompare(b.needDate)||a.sku.localeCompare(b.sku)||a.orderNo.localeCompare(b.orderNo));pendingByLevel.delete(level);
    for(const demand of mergeEvents(batch)){
      ...
      const key=`${demand.sku}|${demand.warehouse}`,item=itemRecord(state,demand.sku,itemByCode);
      ...
      const safety=num(item.safetyStock),gross=round(demand.grossRequired),shortage=round(Math.max(0,gross+safety-available)),plannedQuantity=shortage?lotSize(shortage,item):0,projected=round(available+plannedQuantity-gross),lead=leadDays(state,graph,demand.sku,itemByCode),releaseDate=plannedQuantity?addDays(demand.needDate,-lead):'',action=product?'MAKE':'BUY';

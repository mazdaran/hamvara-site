...
import {allocateFefoLots} from './advanced-manufacturing.js';
...
export function issueProductionJob(state,jobId,issuedAt=new Date().toISOString(),meta={}){
  ...
  for(const material of job.materials){const stock=state.stock[material.sku],item=state.skus.find(value=>value.code===material.sku),allocations=item?.issuePolicy==='FEFO'?allocateFefoLots(state,{sku:material.sku,warehouse:material.warehouse,quantity:material.required,onDate:issuedAt.slice(0,10)}):[];for(const allocation of allocations){const lot=state.inventoryLots.find(value=>value.sku===material.sku&&value.warehouse===material.warehouse&&value.lotNo===allocation.lotNo&&value.serialNo===allocation.serialNo);lot.quantity=number(lot.quantity)-allocation.quantity}stock[material.warehouse]=number(stock[material.warehouse]||0)-material.required;stock.reserved=Math.max(0,number(stock.reserved||0)-material.required);stock['WH-SF']=number(stock['WH-SF']||0)+material.required;appendInventoryMovement(state,{...metaFields(meta),sku:material.sku,warehouse:material.warehouse,qty:material.required,direction:'OUT',type:'PRODUCTION_ISSUE',reference:job.workOrderNo,batchNo:job.batchNo,at:issuedAt,note:allocations.map(value=>`${value.lotNo}:${value.quantity}`).join(', ')});appendInventoryMovement(state,{...metaFields(meta),sku:material.sku,warehouse:'WH-SF',qty:material.required,direction:'IN',type:'SHOP_FLOOR_RECEIPT',reference:job.workOrderNo,batchNo:job.batchNo,at:issuedAt})}
